...
var Scoreboard = function(game) {
  ...
  this.scoreboard = this.create(this.game.width / 2, 200, 'scoreImg');
  this.scoreboard.anchor.setTo(0.5, 0.5);
  this.scoreboard.fixedToCamera = true;
  
  this.scoreText = this.game.add.bitmapText(this.scoreboard.x - 20 + this.scoreboard.width/2, 180, 'flappyfont', '', 18);
  this.scoreText.fixedToCamera = true;
  this.scoreText.anchor.setTo(1, 0);
  this.add(this.scoreText);
  
  this.bestText = this.game.add.bitmapText(this.scoreboard.x - 20 + this.scoreboard.width/2, 230, 'flappyfont', '', 18);
  this.bestText.fixedToCamera = true;
  this.bestText.anchor.setTo(1, 0);
...
Scoreboard.prototype.show = function(score) {
  ...
  if(!!localStorage) {
    ...
    if(!bestScore || bestScore < score) {
	  ...
	  //enviamos la puntuacion de nuestro pajaro
